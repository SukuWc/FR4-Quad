...
void nRF24_DumpConfig(void) {
	...
	uint8_t strbuff[2048];

	// Dump nRF24L01+ configuration
	// CONFIG
	i = nRF24_ReadReg(nRF24_REG_CONFIG);
	snprintf(strbuff, 2048, "[0x%02X] 0x%02X MASK:%03b CRC:%02b PWR:%s MODE:P%s\r\n",
			nRF24_REG_CONFIG,
			i,
			i >> 4,
			(i & 0x0c) >> 2,
	...
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X ENAA: ",nRF24_REG_EN_AA,i);
	printf("%s", strbuff);
	for (j = 0; j < 6; j++) {
		snprintf(strbuff, 2048,"[P%1u%s]%s",j,
				(i & (1 << j)) ? "+" : "-",
				(j == 5) ? "\r\n" : " "
			);
		printf("%s", strbuff);
	...
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X EN_RXADDR: ",nRF24_REG_EN_RXADDR,i);
	printf("%s", strbuff);
	for (j = 0; j < 6; j++) {
		snprintf(strbuff, 2048,"[P%1u%s]%s",j,
				(i & (1 << j)) ? "+" : "-",
	...
	aw = (i & 0x03) + 2;
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X EN_RXADDR=%06b (address width = %u)\r\n",nRF24_REG_SETUP_AW,i,i & 0x03,aw);
	printf("%s", strbuff);
	// SETUP_RETR
	i = nRF24_ReadReg(nRF24_REG_SETUP_RETR);
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X ARD=%04b ARC=%04b (retr.delay=%uus, count=%u)\r\n",
	...
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X (%.3uGHz)\r\n",nRF24_REG_RF_CH,i,2400 + i);
	printf("%s", strbuff);
	// RF_SETUP
	i = nRF24_ReadReg(nRF24_REG_RF_SETUP);
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X CONT_WAVE:%s PLL_LOCK:%s DataRate=",
			nRF24_REG_RF_SETUP,
			i,
			(i & 0x80) ? "ON" : "OFF",
			(i & 0x80) ? "ON" : "OFF"
	...
	switch ((i & 0x28) >> 3) {
		case 0x00:
			snprintf(strbuff, 2048,"1M");
			printf("%s", strbuff);
			break;
		case 0x01:
			snprintf(strbuff, 2048,"2M");
			printf("%s", strbuff);
			break;
		case 0x04:
			snprintf(strbuff, 2048,"250k");
			printf("%s", strbuff);
			break;
		default:
			snprintf(strbuff, 2048,"???");
			printf("%s", strbuff);
			break;
	}
	snprintf(strbuff, 2048,"pbs RF_PWR=");
	printf("%s", strbuff);
	switch ((i & 0x06) >> 1) {
		case 0x00:
			snprintf(strbuff, 2048,"-18");
			printf("%s", strbuff);
			break;
		case 0x01:
			snprintf(strbuff, 2048,"-12");
			printf("%s", strbuff);
			break;
		case 0x02:
			snprintf(strbuff, 2048,"-6");
			printf("%s", strbuff);
			break;
		case 0x03:
			snprintf(strbuff, 2048,"0");
			printf("%s", strbuff);
			break;
		default:
			snprintf(strbuff, 2048,"???");
	...
	snprintf(strbuff, 2048,"dBm\r\n");
	printf("%s", strbuff);
	// STATUS
	i = nRF24_ReadReg(nRF24_REG_STATUS);
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X IRQ:%03b RX_PIPE:%u TX_FULL:%s\r\n",
			nRF24_REG_STATUS,
			i,
			(i & 0x70) >> 4,
			(i & 0x0E) >> 1,
	...
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X PLOS_CNT=%u ARC_CNT=%u\r\n",nRF24_REG_OBSERVE_TX,i,i >> 4,i & 0x0F);
	printf("%s", strbuff);
	// RPD
	i = nRF24_ReadReg(nRF24_REG_RPD);
	snprintf(strbuff, 2048,"[0x%02X] 0x%02X RPD=%s\r\n",nRF24_REG_RPD,i,(i & 0x01) ? "YES" : "NO");
	printf("%s", strbuff);
	// RX_ADDR_P0
	nRF24_ReadMBReg(nRF24_REG_RX_ADDR_P0,buf,aw);
	snprintf(strbuff, 2048,"[0x%02X] RX_ADDR_P0 \"",nRF24_REG_RX_ADDR_P0);
	printf("%s", strbuff);
	for (i = 0; i < aw; i++){ snprintf(strbuff, 2048,"%c",buf[i]);
		printf("%s", strbuff);}
	snprintf(strbuff, 2048,"\"\r\n");
	printf("%s", strbuff);
	// RX_ADDR_P1
	nRF24_ReadMBReg(nRF24_REG_RX_ADDR_P1,buf,aw);
	snprintf(strbuff, 2048,"[0x%02X] RX_ADDR_P1 \"",nRF24_REG_RX_ADDR_P1);
	for (i = 0; i < aw; i++) {snprintf(strbuff, 2048,"%c",buf[i]);
	printf("%s", strbuff);}
	snprintf(strbuff, 2048,"\"\r\n");
	printf("%s", strbuff);
	// RX_ADDR_P2
	snprintf(strbuff, 2048,"[0x%02X] RX_ADDR_P2 \"",nRF24_REG_RX_ADDR_P2);
	printf("%s", strbuff);
	for (i = 0; i < aw - 1; i++) {snprintf(strbuff, 2048,"%c",buf[i]);
	printf("%s", strbuff);}
	i = nRF24_ReadReg(nRF24_REG_RX_ADDR_P2);
	snprintf(strbuff, 2048,"%c\"\r\n",i);
	printf("%s", strbuff);
	// RX_ADDR_P3
	snprintf(strbuff, 2048,"[0x%02X] RX_ADDR_P3 \"",nRF24_REG_RX_ADDR_P3);
	for (i = 0; i < aw - 1; i++) {snprintf(strbuff, 2048,"%c",buf[i]);
	printf("%s", strbuff);}
	...
	printf("%s", strbuff);
	// RX_ADDR_P4
	snprintf(strbuff, 2048,"[0x%02X] RX_ADDR_P4 \"",nRF24_REG_RX_ADDR_P4);
	printf("%s", strbuff);
	for (i = 0; i < aw - 1; i++) {snprintf(strbuff, 2048,"%c",buf[i]);
	printf("%s", strbuff);}
	...
	printf("%s", strbuff);
	// RX_ADDR_P5
	snprintf(strbuff, 2048,"[0x%02X] RX_ADDR_P5 \"",nRF24_REG_RX_ADDR_P5);
	printf("%s", strbuff);
	for (i = 0; i < aw - 1; i++) {snprintf(strbuff, 2048,"%c",buf[i]);
	printf("%s", strbuff);}
	...
	snprintf(strbuff, 2048,"[0x%02X] TX_ADDR \"",nRF24_REG_TX_ADDR);
	printf("%s", strbuff);
	for (i = 0; i < aw; i++) {snprintf(strbuff, 2048,"%c",buf[i]);
	printf("%s", strbuff);}
	snprintf(strbuff, 2048,"\"\r\n");
